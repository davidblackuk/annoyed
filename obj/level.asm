...
;src/level.c:56: res = LevelCompleteFail;
	...
	ld	0 (iy), #0x02
00105$:
;src/level.c:58: if (blocks_remaining == 0) {
	ld	iy, #_blocks_remaining
	ld	a, 1 (iy)
	or	a,0 (iy)
	jr	NZ,00107$
;src/level.c:59: res = LevelCompleteSuccess;
	ld	iy, #0
	add	iy, sp
	ld	0 (iy), #0x03
00107$:
;src/level.c:62: return res;
	ld	iy, #0
	add	iy, sp
	ld	l, 0 (iy)
	inc	sp
	ret
;src/level.c:70: void level_initialize_internal(u8 is_restart)
;	---------------------------------
; Function level_initialize_internal
; ---------------------------------
_level_initialize_internal::
;src/level.c:72: cpct_clearScreen(AN_PEN_BLACK);
	ld	hl, #0x4000
	push	hl
	xor	a, a
	push	af
	...
	push	hl
	call	_cpct_memset
;src/level.c:74: background_initialize();
	call	_background_initialize
;src/level.c:75: blocks_initialize(is_restart);
	ld	hl, #2+0
	add	hl, sp
	ld	a, (hl)
	push	af
	...
	inc	sp
;src/level.c:76: auto_initialize();
	call	_auto_initialize
;src/level.c:78: keys_initialize();
	call	_keys_initialize
;src/level.c:79: bat_initialize();
	call	_bat_initialize
;src/level.c:80: balls_initialize();
