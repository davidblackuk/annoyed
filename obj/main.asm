...
;src/main.c:29: initializeCpc();
	call	_initializeCpc
;src/main.c:30: module_menu_initialize();
	call	_module_menu_initialize
;src/main.c:31: module_game_initialize();
	call	_module_game_initialize
;src/main.c:34: while (1) {
00102$:
;src/main.c:35: play_scene(&scene_menu);   
	ld	hl, #_scene_menu
	push	hl
	call	_play_scene
;src/main.c:36: play_scene(&scene_game);   
	ld	hl, #_scene_game
	ex	(sp),hl
	call	_play_scene
	pop	af
	jr	00102$
;src/main.c:41: void play_scene(Scene *scene) {
;	---------------------------------
; Function play_scene
; ---------------------------------
_play_scene::
	...
	add	ix,sp
	push	af
;src/main.c:44: scene->initialize();
	ld	c,4 (ix)
	ld	b,5 (ix)
	ld	l, c
	ld	h, b
...
;src/main.c:53: scene->draw();
	...
	call	___sdcc_call_hl
	pop	bc
;src/main.c:54: state = scene->update();
	ld	l, c
	ld	h, b
	ld	de, #0x0004
	add	hl, de
	...
	call	___sdcc_call_hl
	pop	bc
;src/main.c:59: for (i16 i=0; i< 2000; i++)
	ld	de, #0x0000
00106$:
	ld	a, e
	sub	a, #0xd0
	...
	jr	00106$
00103$:
;src/main.c:66: } while (state == Continue);
	ld	a, l
	or	a, a
	jr	Z,00102$
	ld	sp, ix
